...
export default function PytheniansPage() {
  ...
  // For production, ensure consistent data handling
  const safeRolesNFT = Array.isArray(rolesNFT) ? rolesNFT : [];
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="text-gray-400 border-gray-600 text-sm px-2 py-0.5 rounded-md border">
              {safeRolesNFT.length} Total Roles
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {Array.from({ length: Math.min(8, safeRolesNFT.length) }).map(
            (_, i) => (
              <div
                key={i}
  ...
  return <NFTRoles nftRoles={safeRolesNFT} />;
}

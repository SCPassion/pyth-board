...
export function GovernanceStakersPanel() {
  ...
  const latest = useQuery(api.pythGovernanceStakers.latest, {});
  const { results, status, loadMore } = usePaginatedQuery(api.pythGovernanceStakers.history, {since}, {initialNumItems:366});
  useEffect(() => { if (status === "CanLoadMore") loadMore(366); },[status,loadMore]);
  const ready = status === "Exhausted";
  const first = results[0];
  const last = results.at(-1);
  const change = ready && results.length >= 2 && last && first ? last.stakers-first.stakers : null;
  const chart = results.flatMap((row,i) => {
    const time = Date.parse(`${row.date}T00:00:00Z`);
    const point = {time,stakers:row.stakers};
    // Break the line at missed days without inventing staker values.
    return i > 0 && time-Date.parse(`${results[i-1].date}T00:00:00Z`) > DAY
      ? [{time:time-DAY,stakers:null},point] : [point];
  });
  const stale = latest && now-latest.collectedAt > 36*60*60*1000;
  return <section className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6" aria-labelledby="governance-staker-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 id="governance-staker-title" className="text-sm font-medium text-white/80">Governance Stakers</h2>
          <p className="mt-1 text-xs text-white/60">Voting-eligible owners · Daily at 15:00 UTC</p>
          <p className="mt-3 text-4xl font-medium tracking-tight tabular-nums text-white sm:text-5xl">{latest ? number.format(latest.stakers) : "—"}</p>
        ...
        <div className="flex gap-1 rounded-xl border border-white/15 p-1" aria-label="Governance staker history period">
      ...
      <p className="mt-5 min-h-10 text-xs leading-relaxed text-white/80">{change !== null && first ? <><span className={change>=0 ? "text-violet-300" : "text-rose-300"}>{change>0?"+":""}{number.format(change)} stakers ({change>0?"+":""}{(100*change/first.stakers).toFixed(2)}%)</span> · across available {period === "ALL" ? "history" : period+" history"}</> : ready ? "Period change will appear after two daily collections." : "Loading history…"}</p>
      {ready && chart.length === 0 ? <div className="flex mt-4 h-60 items-center justify-center text-center text-sm text-white/65">{latest ? "No snapshots in this period." : "Your history starts with the first successful collection. No earlier data is backfilled."}</div> :
        <ChartContainer config={{stakers:{label:"Governance Stakers",color:"#c4b5fd"}}} className="mt-4 h-60 w-full aspect-auto">
